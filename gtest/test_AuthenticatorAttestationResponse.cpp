...
class AuthenticatorAttestationResponseTest : public ::testing::Test {
protected:
  std::shared_ptr<Json::Value> json;
  AuthenticatorAttestationResponse attr;
  AttestationStatementFormatIdentifier fmt{
      AttestationStatementFormatIdentifier::fido_u2f};
...
TEST_F(AuthenticatorAttestationResponseTest,
       CheckDeserializedAttestationStatementFormat) {
  ASSERT_EQ(this->fmt, *this->attr.getFmt())
      << "The Attestation Statement Format should have been fido_u2f but was "
      << *this->attr.getFmt()->getString();
}
